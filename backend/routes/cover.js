...
router.post("/", checkAICallQuotaRedis, async (req, res) => {
    try {
        const { jobDescription, company, role, source, style } = req.body; 

        if (!jobDescription || !company || !role) {
            return res.status(400).json({ success: false, message: "jobDescription, company, and role are required." });
        }
        ...
        if (!profile.cvBullets?.length) {
            return res.status(400).json({ success: false, message: "No CV bullets found. Please re-upload your resume." });
        }
        ...
        if (err.status === 429) {
            return res.status(429).json({ success: false, message: "AI quota exceeded. Please wait and try again.", code: "QUOTA_EXCEEDED" });
        }
